...
Let's visualize the data. Dots are cells with velocity (y-axis). Each facet is 
a compound. The x-axis represent doses. There are three plates (color code). 
Four technical replicates (wells) analyzed on the same plate are stacked next 
to each other.
...
## What are the **overall treatment effects** ($\delta$) on cell migration velocity?

* Dot = mean $\delta$
* Error bar = 95% HDI of $\delta$
