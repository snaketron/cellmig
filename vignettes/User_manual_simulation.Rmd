...
`r Biocpkg("ClustIRR")` R package implements a Bayesian hierarchical model 
for analyzing such data. See the "User manual" vignette to understand the
key functionalities of `r Biocpkg("ClustIRR")`.

Furthermore, `r Biocpkg("ClustIRR")` includes functionality for simulating
...
This vignette demonstrates how to use `r Biocpkg("ClustIRR")` to simulate 
...
then analyze the simulated data using `r Biocpkg("ClustIRR")` to recover the
...
We use the `gen_partial` function from `r Biocpkg("ClustIRR")` to simulate the data.
...
### Analyzing simulated data with `r Biocpkg("ClustIRR")`

The simulated data must be formatted to match the input requirements of
`r Biocpkg("ClustIRR")`. Specifically, we need to convert some columns to character
...
We now analyze the simulated data using `r Biocpkg("ClustIRR")` with the following
control parameters:
...
We can also use `r Biocpkg("ClustIRR")` to generate cell velocities from 
