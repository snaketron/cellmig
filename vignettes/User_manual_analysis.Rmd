...
on the y-axis. Each facet corresponds to a compound. The x-axis
represents the dose. There are three plates, indicated by color. Four
technical replicates (wells), analyzed on the same plate, are stacked
next to each other.
...
Next, we fit the Stan model employed by `cellmig` with the control parameters
...
## What are the **overall treatment effects** (parameter $\delta$) on velocity?
...
and overlapping doses, we can study the dose-response profiles by 
hierarchical clustering based on the entire $\delta$ posteriors to 
