...
for(c in 1:length(mu_compound)) {
  for(d in 1:length(mu_dose)) {
    ...
    for(p in 1:length(alpha_plate)) {
      # if plate <= 2 -> experiment 1, else experiment 2
      e <- ifelse(test = p<=2, yes = 1, no = 2)
      u <- rnorm(n = n_reps, mean = alpha_experiment[e] + alpha_plate[p] + mu_group, sd = sigma_bplate)
