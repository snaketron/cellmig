data {
  int<lower=0> N_experiment;                  // number of experiments
  int<lower=0> N_plate;                       // number of plates per experiment
  ...
  vector [N_experiment] alpha_experiment;     // experiment-specific intercept
  matrix [N_plate, N_experiment] alpha_plate;  // exp-plate-specific intercept
  vector [N_group] mu_group;                  // average effect of groups
}

transformed data {
  int<lower=0> N_well = N_experiment * N_plate * N_group * N_well_reps;
}
...
generated quantities {
  ...
  well_id = 1;
  for(experiment_index in 1:N_experiment) {
    for(plate_index in 1:N_plate) {
      ...
      rate =  exp(mu_well + alpha_plate[plate_index,experiment_index] + alpha_experiment[experiment_index]);
    }
  }
